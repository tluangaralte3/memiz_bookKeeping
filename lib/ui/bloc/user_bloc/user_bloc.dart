...
import 'package:memiz_bk/domain/repository/secure_storage_repository.dart';
...
class UserBloc extends Bloc<UserEvent, UserState> {
  ...
  Future<void> validateUser(
    ...
    if (await secureStorageRepository.checkAvailability(email)) {
      emit(state.copyWith(
          ...
          errorMessage: 'User already exists')); // Direct string replacement
      return;
    }
  ...
  Future<void> signIn(Emitter emit, String email) async {
    ...
    if (user == null) {
      emit(state.copyWith(
          status: AuthStatus.error,
          errorMessage: 'No user found')); // Direct string replacement
    } else {
